...
![Version](https://img.shields.io/badge/version-3.0.1--Maintenance-bd00ff)
![SillyTavern](https://img.shields.io/badge/SillyTavern-Extension-blue)

一个为人格化助手“莉莉丝”设计的 SillyTavern (酒馆) 扩展组件。本项目已全面升级至 **v3.0.1 正式版**。

> **v3.0.1 更新内容**：
> 1. **💾 存储路径服务器化**：彻底摒弃浏览器 `localStorage`，所有数据（好感度、理智值、配置等）统一存储至酒馆服务端，支持多设备无缝同步。
> 2. **🔄 智能更新重构**：更新插件时新增“版本回落检测”轮询，确保云端文件完全写入磁盘后再自动刷新网页，彻底解决代码未加载即刷新的问题。
> 3. **⚡ 体验平滑化**：移除所有更新确认弹窗，实现真正的“一键秒级更新”。
> 4. **⚙️ 默认参数校正**：将动态内容构思间隔默认值修正为 20 分钟（原 240），更符合活跃聊天场景。
